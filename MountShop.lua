...
-- SUBSTITUIR a função MountShopDebugPrint local por esta versão segura:
local function MountShopDebugPrint(...)
    -- Verificação segura para evitar nil access
    local debugMode = false
    
    if ClickMorphMountShop and 
       ClickMorphMountShop.mountShopSystem and
       ClickMorphMountShop.mountShopSystem.settings then
        debugMode = ClickMorphMountShop.mountShopSystem.settings.debugMode
    ...
    if debugMode then
        ...
        print("|cff00ccffMountShop:|r", table.concat(args, " "))
    end
end
